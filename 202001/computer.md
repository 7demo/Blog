...
ip地址划分
    ...
    网络号：主机号。网络号是唯一的，主机号在网段中是唯一的。

    A类地址：8位网络号、24位主机号。前1位为0。可指派的网括号有2的7次方，但是要减去全是0与0111111（即127）。可以有的主机号有2的24次方，减去全是0与全是1.所以，整个A类地址有2的31次方个，占有50%。

    B类地址：16位网络号、16位主机号。前两位为10。可指派的网络地址有2的14次方减去1（128.0.0.0），主机号有2的16次方个，减去全是0与全是1。战友整个的25%。

    C类地址：24位主机号、8位主机号。前三位为110。可指派的网路有2的21次方个，减去1（192.0.0.0）。主机数为2的8次方个减去去全是0与1。占有整个的12.5%。

    D类地址: 前四位为1110，多播地址。

    E类地址：前四位为1111。

点分十进制记法。划分子网号后组成为：网络号、子网号、主机号。子网号的位数是占用的原主机号的。
...
查询DNS分别先查：根域名服务器、顶级域名服务器、权限域名服务器、本地域名服务器。解析过程：递归查询、迭代查询

http是面向事务的。http每个请求的时间是两个RTT(一个三次握手，一个请求传输)加上传输时间。持续连接有两种方式：非流水线方式、流水线方式。

    非流水线方式：建立链接后，每收到响应再去请求对象。省略了一个创立连接的RTT，但是服务器每次响应后是空闲状态。

    流水线方式：不等响应直接连续发送请求对象。
